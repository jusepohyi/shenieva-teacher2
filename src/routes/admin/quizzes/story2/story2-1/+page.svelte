<script lang="ts">
    ...
    let quizzes: Quiz[] = [];
    let story = "story2-1"; // Store-specific identifier
    ...
    async function addQuiz() {
        if (
            ...
            try {
                const response = await fetch(
                    `http://localhost/shenieva-teacher/src/lib/api/add_quiz2.php?story=${story}&store=store1`,
                    {
                        method: "POST",
...
<div class="p-6 bg-white dark:bg-gray-900 shadow-lg rounded-lg">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-orange-600">Hector's Health - Quizzes</h2>
        <button
